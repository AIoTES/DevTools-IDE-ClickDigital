 ...
 * ---------------------------------------------------------------------
 */
package anomalymanager.nupic.network;
...
import anomalymanager.nupic.algorithms.Classification;
...
import anomalymanager.nupic.model.SDR;
import anomalymanager.nupic.network.sensor.FileSensor;
import anomalymanager.nupic.network.sensor.HTMSensor;
...
import anomalymanager.nupic.network.sensor.URISensor;
import anomalymanager.nupic.util.ArrayUtils;
import anomalymanager.nupic.util.NamedTuple;
import anomalymanager.nupic.util.SparseBinaryMatrix;
...
public class Layer<T> implements Persistable {
    ...
    public Layer(Parameters params, MultiEncoder e, SpatialPooler sp, TemporalMemory tm, Boolean autoCreateClassifiers, Anomaly a) {
        ...
        if(params.get(Parameters.KEY.FIELD_ENCODING_MAP) == null && e != null) {
            throw new IllegalArgumentException("The passed in Parameters must contain a field encoding map " + 
                "specified by anomalymanager.nupic.Parameters.KEY.FIELD_ENCODING_MAP");
        }
    ...
    public Layer<T> close() {
        ...
        if(encoder != null) {
            if(encoder.getEncoders(encoder) == null || encoder.getEncoders(encoder).size() < 1) {
                if(params.get(Parameters.KEY.FIELD_ENCODING_MAP) == null || ((Map<String, Map<String, Object>>)params.get(Parameters.KEY.FIELD_ENCODING_MAP)).size() < 1) {
                ...
                encoder.addMultipleEncoders((Map<String, Map<String, Object>>)params.get(Parameters.KEY.FIELD_ENCODING_MAP));
            }
            ...
            if(((inputLength = ((int[])params.get(Parameters.KEY.INPUT_DIMENSIONS)).length) != (columnLength = ((int[])params.get(Parameters.KEY.COLUMN_DIMENSIONS)).length))
                            || encoder.getWidth() != (product = ArrayUtils.product((int[])params.get(Parameters.KEY.INPUT_DIMENSIONS)))) {
        ...
        autoCreateClassifiers = autoCreateClassifiers != null && (autoCreateClassifiers | (Boolean)params.get(Parameters.KEY.AUTO_CLASSIFY));
        ...
        if(spatialPooler != null) {
            ...
            if((inputLength = ((int[])params.get(Parameters.KEY.INPUT_DIMENSIONS)).length) !=
                (columnLength = ((int[])params.get(Parameters.KEY.COLUMN_DIMENSIONS)).length)) {
     ...
     * {@link #alterParameter(Parameters.KEY, Object)} method to change a local setting
     * without impacting the same setting in the source parameters object. This
     * is made possible because the {@link #alterParameter(Parameters.KEY, Object)} method
    ...
    public Layer<T> alterParameter(Parameters.KEY key, Object value) {
        ...
        // in
        // previous layers
        int[] inputDims = (int[])params.get(Parameters.KEY.INPUT_DIMENSIONS);
    ...
    private Observable<ManualInput> resolveObservableSequence(T t) {
        ...
        if(recordNum > 0 && skip != -1) {
            ...
            Integer skipCount;
            if(((skipCount = (Integer)params.get(Parameters.KEY.SP_PRIMER_DELAY)) != null)) {
                ...
                params.set(Parameters.KEY.SP_PRIMER_DELAY, Math.max(0, skipCount - recordNum));
            }
        }
     ...
     * by delegating the call to {@link Observer#onNext}(byte[]) of all the currently queued
    ...
    NamedTuple makeClassifiers(MultiEncoder encoder) {
        Map<String, Class<? extends Classifier>> inferredFields = (Map<String, Class<? extends Classifier>>) params.get(Parameters.KEY.INFERRED_FIELDS);
    ...
    /**
     * Returns an {@link Observable} operator that when subscribed to, invokes an operation
     ...
     * and register a listener at the same time. The {@link Observer} will be
    ...
    static class CheckPointOperator<T> extends Observable<T> implements CheckPointOp<T> {
         ...
         */
        protected CheckPointOperator(OnSubscribe<T> f) {
            super(f);
        }
        
        /**
         * Queues the specified {@link Observer} for notification upon
    ...
    private void recreateSensors() {
        if(sensor != null) {
            ...
            if(sensorKlass.toString().indexOf("File") != -1) {
                ...
                sensor = (HTMSensor<?>)Sensor.create(
                    ObservableSensor::create, SensorParams.create(SensorParams.Keys::obs, "", supplierOfObservable));
