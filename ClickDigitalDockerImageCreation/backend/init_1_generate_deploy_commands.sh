...
deploy() {
	...
	echo DEPLOY_STATEMENT=$DEPLOY_STATEMENT
	DEPLOY_STATEMENT="deploy --contextroot=/development/clickdigital/api /opt/payara/deployments/ClickDigitalBackend_war_exploded"
	echo DEPLOY_STATEMENT=$DEPLOY_STATEMENT
	echo DEPLOY_PROPS=$DEPLOY_PROPS
	#DEPLOY_STATEMENT="deploy $DEPLOY_PROPS $1"
